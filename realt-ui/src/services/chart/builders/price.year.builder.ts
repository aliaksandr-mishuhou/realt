...
export class PriceYearBuilder implements LineChartBuilder {

  private mapCallback: (i: Item) => any = i => i.price;
  ...
  build(search: Search, onSuccess: (data: LineChartData) => void): void {
    ...
    const result = this.statsService.getDailyPriceWithYear(search);
