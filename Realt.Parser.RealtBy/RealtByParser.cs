...
namespace Realt.Parser.RealtBy
{
    public class RealtByParser : IParser
    {
        private const string DefaultParamsConfigPath = "./search.params";
        ...
        private readonly ILogger<RealtByParser> _logger;
        private readonly string _paramsConfigPath;

        public int Source => SourceId;

        public RealtByParser(ILogger<RealtByParser> logger, string paramsConfigPath)
        {
            _logger = logger;

            if (string.IsNullOrEmpty(paramsConfigPath))
            {
                paramsConfigPath = DefaultParamsConfigPath;
            }

            _paramsConfigPath = paramsConfigPath;
        }

        public IEnumerable<Search> GetSearchSequence()
        {
            return new RealtBySequence();
        }
        ...
        private async Task<Dictionary<string, string>> LoadSearchParamsAsync(Search search = null)
        {
            // collect params
            var formParams = new Dictionary<string, string>();

            var lines = await File.ReadAllLinesAsync(_paramsConfigPath);
